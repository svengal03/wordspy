...
export function WordspyOffline() {
  ...
  function handleNewGame() {
    // Reset game back to lobby in-place — no navigation needed and avoids the
    // "already on this route → useEffect won't re-run → stuck on loading" bug.
    // Players are kept so nobody has to re-type their names.
    const freshState = createInitialGameState("OFFLINE", config);
    setGameState({
      ...freshState,
  ...
  useEffect(() => {
    if (!localPlayer) { router.push("/wordspy"); return; }
        ...
        <TopBar
          ...
          right={
            <div style={{ display: "flex", gap: 8 }}>
              <NavBtn onClick={() => setShowHelp(true)}>Rules</NavBtn>
              <OptionsMenu onExit={() => { reset(); router.push("/wordspy"); }} />
            </div>
          }
        />
    ...
    return (
      ...
      <Screen>
        <TopBar
          ...
          right={<OptionsMenu onExit={handleLeave} onNewGame={handleNewGame} />}
        />
    ...
    return (
      ...
      <Screen>
        <TopBar
          appName="Wordspy"
          title="Votes Tied"
          sub="Host must choose who is eliminated"
          right={<OptionsMenu onExit={handleLeave} onNewGame={handleNewGame} />}
        />
  ...
  if (gameState.phase === "summary") {
    const handlePlayAgain = () => {
      // Same in-place reset as handleNewGame — avoids stuck loading screen
      const freshState = createInitialGameState("OFFLINE", config);
      setGameState({
        ...freshState,
        players: gameState.players.map((p) => ({
