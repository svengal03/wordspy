...
import { TIMER_OPTIONS } from "@playhub/core";
...
interface Props<T> {
  ...
  tagline: ReactNode;
  description: string;
  buildTeam: (draft: TeamDraft, idx: number) => T;
  onStart: (teams: T[], timerDuration: number, selectedPackIds: string[]) => void;
  onNewGame?: () => void;
  hostName?: string;
...
export function TeamSetupScreen<T>({ appName, game, teamPalette, defaultPackIds, tagline, description, buildTeam, onStart, onNewGame, hostName, rulesModal, initialTeams }: Props<T>) {
  ...
  const [teams, setTeams] = useState<TeamDraft[]>(() =>
    initialTeams && initialTeams.length >= 2
      ? initialTeams.map((t) => ({ id: Math.random().toString(36).slice(2), name: t.name, players: t.players.length > 0 ? t.players : [""] }))
  ...
  function resetForm() {
    ...
    setTimerDuration(60);
    setSelectedPackIds(defaultPackIds);
    setStartError(null);
    onNewGame?.();
  }
  ...
  function addPlayer(ti: number) {
    const newPi = teams[ti]!.players.length;
    setFocusKey(`${ti}-${newPi}`);
  ...
  function handleStart() {
    const builtTeams: T[] = teams.map((t, idx) => buildTeam(t, idx));
    const emptyTeam = (builtTeams as unknown as Array<{ players: string[]; name: string }>).find((t) => t.players.length === 0);
    if (emptyTeam) {
                    ...
                    {team.players.map((p, pi) => (
                      <motion.div key={pi} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 8 }}
                        ...
                        <PlayerNameInput
                          ...
                          onKeyDown={(e) => { if (e.key === "Enter" && pi === team.players.length - 1) addPlayer(ti); }}
                          autoFocus={focusKey === `${ti}-${pi}`}
