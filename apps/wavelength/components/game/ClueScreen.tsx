...
import { Screen, TopBar, Card, Btn, NavBtn, tokens, TEAM_META, RevealCover, OptionsMenu, SectionLabel, RoundBar } from "@/components/ui";

const HOME_URL = process.env.NEXT_PUBLIC_HOME_URL ?? "https://playhub-home.vercel.app";
import { useGame } from "@/lib/store";
import { submitClue } from "@/lib/gameEngine";
...
export default function ClueScreen() {
  ...
  const teamMeta = TEAM_META[game.currentTeamId];
  ...
  if (!revealed) {
    return (
      <Screen>
        <TopBar
          appName="Wavelength"
          right={<OptionsMenu onNewGame={reset} onExit={() => { window.location.href = HOME_URL; }} />}
        />
