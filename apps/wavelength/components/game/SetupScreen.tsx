...
import { Screen, TopBar, Card, Btn, Toggle, NavBtn, tokens, OptionsMenu, SectionLabel } from "@/components/ui";

const HOME_URL = process.env.NEXT_PUBLIC_HOME_URL ?? "https://playhub-home.vercel.app";
import { PlayerNameInput } from "@playhub/ui";
import { useGame } from "@/lib/store";
import { createPlayer, assignTeamsRandomly } from "@/lib/gameEngine";
import { DEFAULT_CONFIG, GameConfig, Player, TeamId } from "@/lib/types";
import { getWavelengthPacks } from "@/lib/spectrumCards";
    ...
    <Screen>
      <TopBar
        ...
        right={
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <OptionsMenu onExit={() => { window.location.href = HOME_URL; }} />
          </div>
        }
      />
      ...
      <div style={{ padding: "24px 20px 40px", display: "flex", flexDirection: "column", gap: 16 }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ fontSize: 30, fontWeight: 800, color: tokens.black, letterSpacing: -1, lineHeight: 1.2, marginBottom: 8 }}>
            Read minds.<br />
            <span style={{ color: tokens.coral }}>Find the wavelength.</span>
          </div>
          <div style={{ fontSize: 14, color: tokens.grey2, lineHeight: 1.6 }}>
            Two teams. One spectrum. Your clue, their guess.
          </div>
        </motion.div>
